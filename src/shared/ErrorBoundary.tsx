...
class ErrorBoundaryInner extends Component<Props, State> {
  ...
  handleSendReport = async () => {
    ...
    // Sanitize error message: strip control chars, limit length
    const rawMsg = this.state.error?.message ?? "Unknown error";
    const safeMsg = rawMsg.replace(/[\r\n\t]/g, " ").slice(0, 200);

    try {
      const { orbit } = await import("../core/api");
      const report = await orbit.collectCrashReport(safeMsg);
      const title = encodeURIComponent(`[crash] ${safeMsg.slice(0, 80)}`);
      const body = encodeURIComponent(`> Auto-generated by Orbit crash reporter\n\n${report}`);
      ...
      console.warn("[ErrorBoundary] Failed to collect crash report:", err);
      const title = encodeURIComponent(`[crash] ${safeMsg.slice(0, 80)}`);
      const body = encodeURIComponent(`> Auto-generated by Orbit crash reporter\n\n## Crash Report\n\n**Error:** \`${safeMsg}\`\n\n(logs unavailable)`);
      window.open(`https://github.com/imadAttar/orbit/issues/new?title=${title}&body=${body}&labels=bug`);
      this.setState({ reportStatus: "sent" });
